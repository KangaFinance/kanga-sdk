import { KANGAMOB_ADDRESS, STOP_LIMIT_ORDER_ADDRESS } from '../constants'
import { mobTypes, name, types } from '../types'
...
export interface MobApprovalMessage {
  warning: string
  user: string
...
export const getTypedDataMob = (message: MobApprovalMessage, chainId: ChainId) => {
  let domain: Domain = {
    name: 'KangaMob V1',
    chainId: chainId,
    verifyingContract: KANGAMOB_ADDRESS[chainId]
  }
  return {
    types: mobTypes,
...
export const getSignatureWithProviderKangamob = async (
  message: MobApprovalMessage,
  chainId: ChainId,
  provider: Web3Provider
): Promise<{ v: number; r: string; s: string }> => {
  const typedData = getTypedDataMob(message, chainId)
  const signature = await provider.send('eth_signTypedData_v4', [message.user, JSON.stringify(typedData)])
  const { v, r, s } = splitSignature(signature)
  return { v, r, s }
}

export const getSignatureMob = async (mobApproval: MobApprovalMessage, chainId: ChainId, privateKey: string) => {
  let domain: Domain = {
    name: 'KangaMob V1',
    chainId: chainId,
    verifyingContract: KANGAMOB_ADDRESS[chainId]
